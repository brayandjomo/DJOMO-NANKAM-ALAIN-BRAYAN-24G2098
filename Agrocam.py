...
# --- CSS PERSONNALISÉ (Inspiré de l'image et fonctionnalités demandées) ---
st.markdown("""
<style>
    /* Global Styles */
    .stApp { background-color: #0c0e12; color: #e0e0e0; }
    
    /* Animation du bouton Analyse IA */
    ...
    /* Style Page Inscription (Similaire à l'image) */
    .auth-container {
    ...
    .auth-title { color: #1a202c; font-size: 28px; font-weight: 800; margin-bottom: 5px; }
    .auth-subtitle { color: #718096; font-size: 14px; margin-bottom: 25px; }
...
if not st.session_state['connecte']:
    ...
    with col_auth:
        st.markdown("<div style='text-align: center; margin-bottom: 20px;'><h1 style='font-size: 50px;'>🌿</h1></div>", unsafe_allow_html=True)
        
        # Bascule Inscription/Connexion
        mode = st.radio("Action", ["Se Connecter", "S'inscrire"], horizontal=True, label_visibility="collapsed")
        
        if mode == "S'inscrire":
            st.markdown("""<div class='auth-container'><div class='auth-title'>Create Account</div><div class='auth-subtitle'>Start your agricultural journey today.</div></div>""", unsafe_allow_html=True)
            with st.form("signup_form"):
                nom = st.text_input("Full Name", placeholder="John Doe")
                email = st.text_input("Email", placeholder="name@work-email.com")
                ...
                if st.form_submit_button("Sign Up", use_container_width=True):
                    ...
                    except: st.error("Utilisateur déjà existant.")
                    db.close()
        else:
            st.markdown("""<div class='auth-container'><div class='auth-title'>Welcome Back</div><div class='auth-subtitle'>Enter your credentials to access dashboard.</div></div>""", unsafe_allow_html=True)
            with st.form("login_form"):
                u_nom = st.text_input("User Name", placeholder="Your name")
...
# --- DASHBOARD ---
    ...
    c1.title(f"🌿 AgroCam Pro : {st.session_state['user_name']}")
    if c2.button("🚪 Déconnexion"):
        st.session_state['connecte'] = False
        st.rerun()

    k_cols = st.columns(4)
    with k_cols[0]: display_kpi_card("Rendement", "1.42 t/ha", "⚖️ Global")
    with k_cols[1]: display_kpi_card("Santé", "Excellent", "✅ Cheptel")
    with k_cols[2]: display_kpi_card("Ventes", "850k FCFA", "📈 Ce mois")
    with k_cols[3]: display_kpi_card("Météo", "24°C", "☀️ Douala")

    tab1, tab2, tab3 = st.tabs(["📝 Ventes", "🐄 Cheptel", "📊 Intelligence"])
    ...
    with tab1:
        ...
        with st.form("v_form"):
            ...
            if st.form_submit_button("Enregistrer"):
                ...
                db.commit(); db.close(); st.success("Enregistré !")
        
        # Toggle Basculable pour le tableau
        show_v = st.toggle("🔍 Afficher l'historique des ventes", key="t1")
        if show_v:
            db = get_connection()
    ...
    with tab2:
        st.subheader("Suivi Cheptel")
        with st.form("c_form"):
            esp = st.selectbox("Espèce", ["Bovin", "Caprin", "Porcin"])
            san = st.select_slider("Santé", options=["Moyen", "Bon", "Excellent"])
            if st.form_submit_button("Ajouter"):
                db = get_connection()
                db.execute("INSERT INTO cheptel (utilisateur, espece, sante) VALUES (?,?,?)", (st.session_state['user_name'], esp, san))
                db.commit(); db.close(); st.success("Ajouté !")
        
        # Toggle Basculable pour le tableau
        show_c = st.toggle("🔍 Afficher le registre cheptel", key="t2")
        if show_c:
            db = get_connection()
    ...
    with tab3:
        st.subheader("Analyse IA & Prédictions")
        # Bouton avec Animation au survol (Clé spécifique 'ai_btn' définie dans le CSS)
        if st.button("🚀 Lancer l'Analyse IA", key="ai_btn", use_container_width=True):
            with st.spinner("L'IA analyse vos cycles de production..."):
                import time; time.sleep(1)
                st.balloons()
                st.success("Analyse terminée : Augmentation de 15% du rendement prévue pour la saison prochaine !")
        
        # Graphiques Altair
        db = get_connection()
        df_ia = pd.read_sql(f"SELECT culture, prix FROM collectes WHERE utilisateur='{st.session_state['user_name']}'", db)
        db.close()
        if not df_ia.empty:
            chart = alt.Chart(df_ia).mark_bar(cornerRadiusTopLeft=10, cornerRadiusTopRight=10).encode(
                x='culture:N', y='sum(prix):Q', color=alt.value("#00c853")
