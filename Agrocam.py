...
# --- CSS PERSONNALISÉ (Nouveau thème Bleu Ciel) ---
st.markdown("""
<style>
    /* Fond de l'application en bleu ciel */
    .stApp {
        background-color: #e3f2fd; /* Bleu ciel très léger */
    }
    
    /* Couleur du texte principal pour la lisibilité */
    ...
    .stTabs [data-baseweb="tab"] {
        ...
        border-radius: 10px 10px 0px 0px; color: #4a5568; border: 1px solid #cbd5e0;
        padding: 10px 25px; font-weight: 600; transition: all 0.3s ease;
    }
    .stTabs [data-baseweb="tab"]:hover { color: #00c853; background-color: #f7fafc; }
    ...
    .kpi-title { color: #718096; font-size: 14px; text-transform: uppercase; margin-bottom: 8px; font-weight: bold; }
    .kpi-value { color: #00c853; font-size: 28px; font-weight: 800; }
    
    /* Boutons */
    .stButton>button { background-color: #00c853; color: white; border-radius: 8px; font-weight: bold; width: 100%; transition: 0.3s; border: none; }
            ...
            with st.form("signup_form"):
                ...
                if st.form_submit_button("S'inscrire"):
                    ...
                    try:
                        ...
                        db.commit()
                        st.success("Compte créé ! Basculez pour vous connecter.")
                    except:
...
# --- APPLICATION PRINCIPALE ---
    ...
    with tab1:
        ...
        if st.checkbox("🔍 Afficher l'historique des ventes"):
            db = get_connection()
    ...
    with tab3:
        ...
        if not df_v_ia.empty:
            ...
            with col_chart_m:
                ...
                scatter = alt.Chart(df_v_ia).mark_circle(size=100, color='#00c853').encode(
            ...
            with col_chart_s:
                ...
                pie = alt.Chart(df_v_ia).mark_arc(innerRadius=50).encode(
                    theta="count():Q", color=alt.Color("culture:N"),
                    tooltip=['culture', 'count()']
                ).properties(height=350)
                st.altair_chart(pie, use_container_width=True)
        else:
            st.info("Saisissez des données pour activer l'analyse.")
