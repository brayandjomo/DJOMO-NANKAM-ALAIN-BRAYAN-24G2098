...
# --- APPLICATION PRINCIPALE ---
    ...
    with tab3:
        ...
        df_c_ia = pd.read_sql(f"SELECT * FROM cheptel WHERE utilisateur='{st.session_state['user_name']}'", db)
        db.close()
        ...
        if not df_v_ia.empty:
            ...
            with col_chart_m:
                ...
                st.write("📊 **Nuage de points : Distribution des Ventes**")
            ...
            with col_chart_s:
                ...
                pie = alt.Chart(df_v_ia).mark_arc(innerRadius=50).encode(
                    ...
                    tooltip=['culture', 'count()']import streamlit as st
import pandas as pd
import sqlite3
import altair as alt
